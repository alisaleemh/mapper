...
function populateInfoWindow(marker, infowindow) {
  ...
  if (infowindow.marker != marker) {
    ...
    function showListing(markerTitle) {
      ...
      if (markerTitle)
      {
        var marker  = markers.filter(function(object){return object.title == markerTitle;});
        console.log(marker);
        marker[0].setMap(map);
      }
    }
    ...
    var viewModel = {
      ...
      listingClickShow: ko.pureComputed(function() {
        ...
        showListing: function (markerObj) {
          ...
          if (markerObj.title)
          {
            var marker  = markers.filter(function(object){return object.title == markerObj.title;});
            marker[0].setMap(map);
          }
        }
      };
